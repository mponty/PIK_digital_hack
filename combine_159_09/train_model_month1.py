...
if CROSS_VALIDATION:
    model = CatBoostRegressor(random_state=19, iterations=1300)
    # model = CatBoostRegressor(random_state=1, iterations=1300, learning_rate=0.03, depth=10)

    local_validation_cutoff = pd.DatetimeIndex(['2018-02-01']).astype(np.int64)[0]
    ...
    for i in range(5):
        ...
        model = CatBoostRegressor(random_state=i, iterations=600)
    ...
    sub.to_csv('submission_month1.csv', index=False)
