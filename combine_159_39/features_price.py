...
def ff(x):
    ...
    if month_before is not None:
        ...
        super_tt_filtered_1 = super_tt_filtered[(super_tt_filtered['date1'] == month_before)]
        super_tt_filtered_2 = super_tt_filtered[(super_tt_filtered['date1'] <= date1)]
        if super_tt_filtered_2.shape[0] > 0:
            price_before = super_tt_filtered_2['price'].values[0]
            price_all_change = price_now - price_before
        else:
            price_all_change = np.nan

        if super_tt_filtered_1.shape[0] > 0:
            price_before = super_tt_filtered_1['price'].values[0]
    ...
    # return pd.Series([bulk_id, spalen, date1, price_all_change, just_bulk_len], index=['bulk_id', 'spalen', 'date1', 'price_all_change', 'just_bulk_len'])
    return pd.Series([bulk_id, spalen, date1, price_all_change, price_rel_change, just_bulk_len],
                     index=['bulk_id', 'spalen', 'date1', 'price_all_change', 'price_rel_change', 'just_bulk_len'])
